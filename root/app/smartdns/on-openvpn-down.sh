...
log -d smartdns "Remove routing for $country vpn: tun: $tun, ip: $ip, gateway: $gateway, port: $port."
...
log -v smartdns "Delete mangle ($country): iptables -D OUTPUT -t mangle -o eth0 -p tcp --dport $https$port -j MARK --set-mark 0x$port"
iptables -D OUTPUT -t mangle -o eth0 -p tcp --dport $https$port -j MARK --set-mark 0x$port

log -v smartdns "Delete mangle ($country): iptables -D OUTPUT -t mangle -o eth0 -p tcp --dport $http$port -j MARK --set-mark 0x$port"
